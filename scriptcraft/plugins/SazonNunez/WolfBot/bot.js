...
var bot = plugin('bot',{
    ...
    summon: function(player){
        ...
        // ??? factory.getEntityFactory();
        
        var entityFactory = Packages.net.canarymod.Canary.factory().entityFactory;
        var my_bot = entityFactory.newEntity(Packages.net.canarymod.api.entity.EntityType.WOLF, self.location);
        my_bot.spawn();
        
        // var my_bot = world.spawnCreature(player.getLocation().add(0,0,2), Packages.net.canarymod.api.entity.EntityType.WOLF);
        
        my_bot.setTamed(true);
        my_bot.setOwner(player);
        my_bot.setAttackTarget(player);
        
        var inventory; 
        var b = this.get_bot(player);
        if(b == null || b.inventory == null) {
            // inventory = player.getServer().createInventory(player, 36, "Bot's Pack");
            inventory = Packages.net.canarymod.api.inventory.AnimalInventory.getInventoryName;
            echo(inventory);    
        } else {
    ...
    pack: function(player){
        ...
        if(b!= null && b.bot !== null){
            b.bot.setSitting(true);
            // http://docs.visualillusionsent.net/CanaryLib/1.1.2/index.html?net/canarymod/api/entity/living/class-use/LivingBase.html
